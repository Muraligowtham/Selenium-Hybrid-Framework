	...
	public static void Engine()throws Exception{
       ...
       //Checking the cases to execute
			...
			System.out.println(testid);
				...
				SSDI_Ts Start=new SSDI_Ts(testid);
						...
						if(cell != null )
						{
							...
							case Cell.CELL_TYPE_NUMERIC:
								...
								{ 
									...
									else if(sh.getRow(0).getCell(col).getStringCellValue().equals("Password")){
										ses.setVariable(sh.getRow(0).getCell(col).getStringCellValue(),String.valueOf(cell.getNumericCellValue()).replace(".0", ""));	
									}
									else{
				...
				for(int j=0;j<i;j++){
					...
					int count=	SSDI_POC.iteratorcount(j,ses.getVariable("TestScenario"));
					
					//calling the methods
					Start.WADMCode(count);
					//checking the status after executing the method
					if(ses.getVariable("Status").equals("Fail")){
						log2.info("Fail");
						ses.setVariable("Status","");
					}
					
					//logging the status of current step
					if(ses.getVariable("InitiateNew").equals("No")){
...
//closing the browser at the last step and setting the new execution
					if(j==(i-1)){
						...
						if(ses.getVariable("Status").equals("Pass")){
